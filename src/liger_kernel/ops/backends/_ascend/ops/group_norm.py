...
def _group_norm_forward_affine_channel_block_size(channels_per_group: int, block_h: int) -> int:
    # Cap at 16 so we can use wider channel tiles when block_h is small (more work per program).
    max_block_ch = min(16, triton.next_power_of_2(channels_per_group))
...
def _group_norm_forward_affine_batch_block_size(block_h: int, block_ch: int) -> int:
    ...
    # Larger budget when tiles are small improves batch-axis occupancy without changing kernel logic.
    return max(1, min(8, 4096 // tile_elems))
...
def _group_norm_forward_single_task_kernel(
    ...
    # Single full-tensor reduction avoids nested reductions and lowers to simpler IR.
...
def _group_norm_forward_stats_kernel(
    ...
    for start in tl.range(0, hidden_size, BLOCK_SIZE_H):
        ...
        X_vals = tl.load(X_ptr + ptrs, mask=mask, other=0.0, cache_modifier=".cg").to(tl.float32)
...
def group_norm_forward(X, num_channels, num_groups, W, B, eps):
    ...
    max_single_task_channels_per_group = 32
    use_single_task = hidden_size <= MAX_FUSED_FORWARD_SIZE and channels_per_group <= max_single_task_channels_per_group

    if use_single_task:
        block_h, block_ch = _group_norm_forward_single_task_block_sizes(
            hidden_size, channels_per_group, X.element_size()
        )
        _group_norm_forward_single_task_kernel[(total_tasks,)](
            Y,
            X,
            Mean,
            RSTD,
